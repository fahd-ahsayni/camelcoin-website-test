import { motion } from "framer-motion";

export default function Circles() {
  return (
    <div className="absolute">
      <motion.div
        initial={{
          opacity: 0,
        }}
        animate={{
          scale: [1, 1.1, 1.3, 1.2, 1],
          opacity: ["20%", "20%", "50%", "80%", "20%"],
        }}
        transition={{
          duration: 2.5,
        }}
        className="relative flex justify-center items-center -mt-40"
      >
        <div className="absolute border border-slate-600 rounded-full h-[200px] w-[200px] mt-52 animate-ping" />
        <div className="absolute border border-slate-600 rounded-full h-[300px] w-[300px] mt-52" />
        <div className="absolute border border-slate-600 opacity-20 rounded-full h-[500px] w-[500px] mt-52" />
        <div className="absolute border border-cyan-800 rounded-full h-[650px] w-[650px] mt-52 animate-pulse" />
        <div className="absolute border border-slate-600 rounded-full h-[800px] w-[800px] mt-52" />
      </motion.div>
    </div>
  );
}
